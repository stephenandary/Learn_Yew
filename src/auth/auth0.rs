...
impl Auth0 {
    ...
    fn create_login_url(state: &str) -> String {
        ...
        format!("https://{domain}/authorize?response_type=token&client_id={client_id}&redirect_uri={redirect_uri}&scope=profile openid email&state={state}&connection=Spotify ")
    }
